...
export default function FinanceiroMesPage() {
  ...
  useEffect(() => {
    const fetchDados = async () => {
      ...
      const relatorioResult = await buscarRelatorioFinanceiro({
        dataInicio,
        dataFim,
      });
      ...
      if (orcamentosResult.success && orcamentosResult.orcamentos) {
        obrasPeriodo = orcamentosResult.orcamentos
          .filter((o: any) => {
          ...
          .map((obra: any) => {
            ...
            const ajudantes = Array.isArray(obra.ajudantes)
              ? obra.ajudantes
              : typeof obra.ajudantes === "string"
              ? (() => {
                  try {
                    return JSON.parse(obra.ajudantes);
                  } catch {
                    return [];
                  }
                })()
            ...
            if (Array.isArray(obra.pagamentos) && obra.pagamentos.length > 0) {
              ...
              const ultimo = obra.pagamentos.reduce((a: any, b: any) =>
                new Date(a.dataVencimento) > new Date(b.dataVencimento) ? a : b
              );
              pagamento = {
                status:
                  ultimo.statusPagamento === "PAGO"
                    ? ("PAGO" as const)
                    : ("NAO_PAGO" as const),
            ...
            pagamentosMap[obra.id] = pagamento || {
  ...
  const handlePagamentoChange = async (
    obraId: string,
    novo: { status: "PAGO" | "NAO_PAGO"; metodo?: string; parcelas?: number }
  ) => {
    // Atualizar no banco
    ...
    <div className="min-h-screen bg-white">
      <div className="container mx-auto px-4 py-8">
        <div className="mt-4 mb-8 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <p className="text-black">Gerencie todos os pagamentos</p>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            {obras.length > 0 && (
              <div className="bg-white text-black rounded shadow-xl p-4">
                <h2 className="text-xl font-semibold mb-4">
                  Valores Detalhados dos Orçamentos
                </h2>
                <div className="space-y-6">
                  {obras.map((obra) => (
                    <ValoresOrcamento
                      ...
                      onChangePagamento={(novo) =>
                        handlePagamentoChange(obra.id, novo)
                      }
            ...
            <PagamentosCard
              ...
              onVerDetalhes={() =>
                router.push(`/financeiro/${mesSelecionado}/pagamentos`)
              }
            />
          </div>
        )}
