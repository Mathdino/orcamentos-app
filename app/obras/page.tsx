...
export default function ObrasPage() {
  ...
  const carregarObras = async () => {
    try {
      ...
      if (resultado.success && resultado.orcamentos) {
        ...
        const obrasFiltradas = resultado.orcamentos.map((o: any) => ({
          id: o.id,
          cliente: {
            nome: o.cliente?.nome || "",
  ...
  const obrasFiltradas = obras.filter((obra) => {
    ...
    const matchStatus =
      statusFiltro === "ALL" || statusFiltro === ""
        ? true
        : obra.status === statusFiltro;
  ...
  return (
    <div className="container mx-auto px-4 py-8 mt-30">
      <div className="h-8" />
      <h1 className="text-3xl font-bold text-black">Obras</h1>
      <p className="text-black">Gerencie todas as obras</p>

      {/* Busca e Filtros */}
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end text-black pt-5 pb-5">
        <Input
      ...
      <div className="grid gap-4">
        ...
        {obrasFiltradas.map((obra) => {
          ...
          return (
            <Card
              key={obra.id}
              className="hover:shadow-2xl shadow-xl transition-shadow"
            >
                ...
                <div className="hidden sm:flex sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                      ...
                      {obra.dataConclusao && (
                        <>
                          ...
                          <span className="text-lg font-bold text-green-700">
                            {new Date(obra.dataConclusao).toLocaleDateString(
                              "pt-BR"
                            )}
                          </span>
                        </>
                    ...
                    <div className="flex-1">
                      ...
                      <p className="flex items-center text-gray-600 mb-2">
                        ...
                        Tipo de serviço: {obra.tipoServico}
                      </p>
                      <span className="text-2xl font-bold text-green-600">
                        {formatarBRL(obra.valorTotal)}
                      </span>
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-2 min-w-[180px]">
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        if (isConcluida || obra.status === "EM_PREPARACAO")
                          return;
                        setLoading(true);
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        if (isConcluida || obra.status === "NAO_INICIADO")
                          return;
                        setLoading(true);
                        const now = new Date().toISOString();
                        const resultado = await atualizarOrcamento(obra.id, {
                  ...
                  <div className="flex flex-col items-start gap-4">
                    ...
                    <div className="w-full">
                      ...
                      <div className="flex flex-col items-start gap-2">
                        <span className="text-2xl font-bold text-green-600 pb-3">
                          {formatarBRL(obra.valorTotal)}
                        </span>
                        <Link
                          href={`/obras/${obra.id}`}
                    ...
                    <div className="flex flex-col items-start gap-2 w-full">
                      <Button
                        ...
                        className="w-full"
                        disabled={
                          isConcluida || obra.status === "EM_PREPARACAO"
                        }
                        onClick={async () => {
                          if (isConcluida || obra.status === "EM_PREPARACAO")
                            return;
